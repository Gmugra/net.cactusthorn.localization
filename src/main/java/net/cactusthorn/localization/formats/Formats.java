...
public class Formats {
    ...
    public Formats(Locale locale, Map<String, String> properties) {
        ...
        for (FormatProperties formatProperties : parse(properties).values()) {
            ...
            case NUMBER:
                numberFormats.put(formatProperties.name, NumberFormatBuilder.buildNumber(locale, formatProperties));
                break;
            case CURRENCY:
                numberFormats.put(formatProperties.name, NumberFormatBuilder.buildCurrency(locale, formatProperties));
                break;
            case INTEGER:
                numberFormats.put(formatProperties.name, NumberFormatBuilder.buildInteger(locale, formatProperties));
                break;
            case PERCENT:
                numberFormats.put(formatProperties.name, NumberFormatBuilder.buildPercent(locale, formatProperties));
                break;
            case DATETIME:
                dateTimeFormats.put(formatProperties.name, DateTimeFormatBuilder.buildDateTime(locale, formatProperties));
                break;
            case DATE:
                dateTimeFormats.put(formatProperties.name, DateTimeFormatBuilder.buildDate(locale, formatProperties));
                break;
            case TIME:
                dateTimeFormats.put(formatProperties.name, DateTimeFormatBuilder.buildTime(locale, formatProperties));
                break;
            }
    ...
    public void combineWith(Formats formats) {
        ...
        if (!this.locale.equals(formats.locale)) {
            return;
        }
    ...
    public String format(String formatName, Object obj) throws LocalizationFormatException {
        ...
        if (dateTimeFormats.containsKey(formatName)) {
            ...
            throw new LocalizationFormatException(locale,
                    "format: \"" + formatName + "\", Unknown class for date/time formatting: " + obj.getClass().getName());
        }

        if (numberFormats.containsKey(formatName)) {

            // to be thread safe we need to create new format object every time,
            // but clone() is more simple to use and also, in this case, working faster
            // (based on JMH tests with JVM 8)
            ...
            try {
                ...
                throw new LocalizationFormatException(locale,
                        "format: \"" + formatName + "\", Unknown class for number formatting: " + obj.getClass().getName(), iae);
            }
        }
